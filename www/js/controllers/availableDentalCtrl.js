...
.controller('availableDentalCtrl', function ($ionicHistory, $scope, $timeout, $http, settings, $stateParams, $state, services) {
    // $scope.onNetworkList = [];
    ...
    services.PlanNameList(settings.PlanConfig.PlanType, settings.PlanConfig.GroupId)
                    .then(function (data) {
    ...
    $scope.expandnetwork = function (index) {
        ...
        if (item.isexpanded_networks) {
            ...
            $timeout(function () {
                ...
                services.PlanAvailable(item.ID)
                  .then(function (data) {
    ...
    $scope.expandplan = function (index, item) {
        ...
        if (item.isexpanded) {
            ...
            $timeout(function () {
                ...
                services.Planrates(item.ID, settings.GroupKey)
                   .then(function (data) {
